...
local Multiplier = SS["MultiplierSetting"].value
...
function refill_turbines()
    for _, surface in pairs(game.surfaces) do
        ...
        for _, turbine in ipairs(turbines) do
            turbine.fluidbox[1] = {name = "EasyWind", amount = Tier_1 * Multiplier, temperature = 100}
            turbine.fluidbox[1] = {name = "EasyWind", amount = Tier_2 * Multiplier, temperature = 100}
            turbine.fluidbox[1] = {name = "EasyWind", amount = Tier_3 * Multiplier, temperature = 100}
            turbine.fluidbox[1] = {name = "EasyWind", amount = Tier_4 * Multiplier, temperature = 100}
            turbine.fluidbox[1] = {name = "EasyWind", amount = Tier_5 * Multiplier, temperature = 100}
            turbine.active = true
        end
    end
end
